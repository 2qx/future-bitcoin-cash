...
describe(`Static Vault Tests`, () => {
    ...
    test("Token address should match", async () => {
        const contract = new Contract(vaultArtifact, [1000n]);
    ...
    test("Token address should match a testnet address", async () => {
        ...
        const contract = new Contract(vaultArtifact, [1000n], {provider});
    ...
    test("Series vault addresses should match", async () => {
        ...
        let addrs = seriesTimes.map( t => { return new Contract(vaultArtifact,[BigInt(t)]).tokenAddress})
    ...
    test("Series coupon addresses should match", async () => {
        ...
        let coupons = bytecodes.map( b => { return new Contract(couponArtifact, [100000000n, binToHex(b)]).tokenAddress})
