...
import { SwapRequestI } from "./interface";
...
describe.skip(`Test placing with coupons`, () => {
    ...
    test.skip("Test redeeming tokens", async () => {
        ...
        let couponAddress = Vault.getCoupon(100_000_000, 200, CashAddressNetworkPrefix.regtest)
        alice.send([{
            cashaddr: alice.getDepositAddress(),
            value: 100_000,
            unit: 'sats',
        ...
        alice.send(
            [new TokenSendRequest({
                cashaddr: Vault.getAddress(200, CashAddressNetworkPrefix.regtest)!,
                amount: 500_000_000n,
                tokenId: tokenId,
                value: 500_001_000,
            }),
    ...
    test.skip("Test high-level swap", async () => {
        ...
        await delay(1000)
        // create a coupon for locking a whole coin in the 1e2 vault
        let couponAddress = Vault.getCoupon(100_000_000, 300, CashAddressNetworkPrefix.regtest)
        alice.send([{
            cashaddr: alice.getDepositAddress(),
            value: 100_000,
            unit: 'sats',
        ...
        alice.send(
            ...
            new TokenSendRequest({
                cashaddr: Vault.getAddress(300, CashAddressNetworkPrefix.regtest)!,
                amount: 200_000_000n,
                tokenId: tokenId,
                value: 200_001_000,
            }),
        ...
        let finalState = await alice.swap(requests, provider)
